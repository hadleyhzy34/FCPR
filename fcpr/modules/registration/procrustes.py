...
def test_weighted_procrustes(src_points, ref_points, pred_pairs, weights=None, weight_thresh=0.0, eps=1e-5, device=torch.device('cpu')):
    ...
    """
    pdb.set_trace()
    src_points = src_points[pred_pairs[:,0],:].unsqueeze(0)  #(1,P,3)
    ref_points = ref_points[pred_pairs[:,1],:].unsqueeze(0)  #(1,P,3)
    weights = weights.unsqueeze(0)  #(1,P)
    
    batch_size = src_points.shape[0]
    weights = torch.where(torch.lt(weights, weight_thresh), torch.zeros_like(weights), weights)
    weights = weights / (torch.sum(weights, dim=1, keepdim=True) + eps)
    weights = weights.unsqueeze(2)  # (1, P, 1)
