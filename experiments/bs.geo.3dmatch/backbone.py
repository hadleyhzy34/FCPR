...
from fcpr.modules.kpconv import ConvBlock, ResidualBlock, UnaryBlock, LastUnaryBlock, nearest_upsample
...
class KPConvFPN(nn.Module):
    ...
    def forward(self, feats, data_dict):
        ...
        # test
        # feats_list.append(feats_s2)  #(add local feature before u-net)
        feats_list.append(latent_s2)

        feats_list.reverse()
        
        # self.timer.append(timeit.default_timer() - startTime)
        # print(f'average model time is: {statistics.mean(self.timer)}')
        return feats_list
